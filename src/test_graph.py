...
def test_del_node(graph):
    ...
    graph.add_node('node')
    graph.del_node('node')
    assert 'node' not in graph.node_dict
...
def test_del_edge(graph):
    ...
    assert 'edon' not in graph.node_dict['node']
...
def test_neighbors(graph):
    ...
    graph.add_edge('bank', 'wheels')
    assert graph.neighbors('bank') == ['car', 'wheels']
...
def test_nodes(graph):
    ...
    graph.add_node('xenophobe')
    assert sorted(graph.nodes()) == ['blah', 'whamo', 'xenophobe']
