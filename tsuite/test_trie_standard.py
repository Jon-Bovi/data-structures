...
from inspect import isgenerator, getmembers, isclass
...
CLASSNAME = next(c[0] for c in classes if 'trie' in c[0].lower())
...
def test_traversal_generator(new_trie):
    ...
    assert isgenerator(new_trie.instance.traversal(new_trie.start))
