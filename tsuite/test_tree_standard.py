...
def _setup_to_delete(cases):
    ...
    for balancing in [False]:
        for sequence in cases:
            if not sequence:
                yield (sequence, None, balancing)
            else:
                for item in sequence:
                    yield (sequence, item, balancing)


@pytest.fixture(scope='function', params=_setup_to_delete(TEST_CASES))
def new_tree(request):
    """Return a new empty instance of MyQueue."""
    sequence, to_delete, balancing = request.param
    instance = ClassDef(autobalance=balancing)
...
def test_traversals(method_name, new_tree):
    ...
    assert list(method(attr='val')) == canon(root)
...
def test_depth_after_delete_half(new_tree):
    ...
    if not new_tree.autobalancing or len(new_tree.sequence) < 2:
        pytest.skip()
