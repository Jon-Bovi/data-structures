...
MODULE_NAME = 'weighted_graph'

module = import_module(MODULE_NAME)

classes = inspect.getmembers(module, predicate=inspect.isclass)
CLASS_NAME = next(c[0] for c in classes if 'graph' in c[0].lower())

ClassDef = getattr(module, CLASS_NAME)
instance = ClassDef()

if issubclass(ClassDef, dict):
    GRAPH_DICT_IMPL = 'extend dict'
else:
    attrs = instance.__dict__
    for attr in attrs:
        if isinstance(getattr(instance, attr), dict):
            GRAPH_DICT_ATTR = attr
            instance.add_edge(1, 2)
            attr_obj = getattr(instance, attr)
            if isinstance(attr_obj, dict):
                GRAPH_DICT_IMPL = 'dict of dicts'
            elif isinstance(attr_obj, list):
                GRAPH_DICT_IMPL = 'dict of list of pairs'
            break

methods = [x[0] for x in inspect.getmembers(ClassDef(), predicate=inspect.ismethod)]
common_algs = [
    'bellman',
    'warshall',
]
try:
    DIJK_NAME = next(m for m in methods if 'dijkstra' in m)
except StopIteration:
    DIJK_NAME = 'dijkstra'
# import pdb; pdb.set_trace()
try:
    alg2_mthd_cands = [m for m in methods if any(alg in m for alg in common_algs)]
    sigs = [inspect.signature(getattr(instance, mthd)).parameters for mthd in alg2_mthd_cands]
    ALG2_NAME = next(name for name, sig in zip(alg2_mthd_cands, sigs) if len(sig) == 2)
except StopIteration:
    ALG2_NAME = 'bellman'
...
def neighbors_with_weights(graph, node):
    """Need to implement."""
    if GRAPH_DICT_IMPL == 'extend dict':
        return graph[node].items()
    neighbors = getattr(GRAPH_DICT_ATTR, graph)[node]
    if GRAPH_DICT_IMPL == 'dict of dicts':
        return neighbors.items()
    elif GRAPH_DICT_IMPL == 'dict of list of pairs':
        return neighbors
...
def test_shortest_path_valid(method_name, traversable_graph):
    ...
    if traversable_graph.error is None:
        ...
        res = result == traversable_graph.result or list(result) == list(traversable_graph.result[1])
        assert res
    else:
